...
config.port = +(process.env.PORT ||
   				process.env.POLLA_PORT || 
				process.argv[2] || 
				config.port || 
				8080);
config.host = process.env.HOST || 
				process.env.POLLA_HOST || 
				process.argv[3] || 
				config.host || 
				'localhost';
config.address = 'http://' + config.host + (process.env.PORT || 
											process.env.POLLA_PORT || 
...
app.use(function(req, res, next) {
	if (req.url === '/') req.url = '/mineorrhea.html';
	next();
});
app.use(app.router);
...
//DMT TODO: Pull this into gameutils
app.exposeModule('lib/keys', 'lib/keys');
//DMT TODO: Refactor gutils
app.exposeModule('middleware/gutils', 'middleware/gutils');

app.exposeModule('mineorrhea_player', 'mineorrhea_player');
...
var Maga = require('maga')
	...
	, MClient = require('mineorrhea_client')
	, GUtils = require('middleware/gutils');

// new game
var game = new Maga.Game('Mineorrhea')
	, room = game.createRoom()
	, players = playerManager(room, MClient);
...
socket.on('connection', function(client) {
	...
	//
	//Send world info somewhere here...
	//
